...
def call_openai_chat_completion(
    ...
    url = build_chat_completions_url(os.environ.get("OPENAI_BASE_URL", "https://api.openai.com"))
...
class CaptionRerankingPipeline:
    ...
    def select_pairwise_central(self, candidates: list[CandidateCaption]) -> list[CandidateCaption]:
        if len(candidates) <= self.top_pairwise_keep:
            return candidates

        embeddings = self.clap_scorer.text_embeddings([candidate.text for candidate in candidates])
    ...
    def rerank(
        ...
        for caption, clap_score, decoder_nll in zip(captions, clap_scores, decoder_nlls):
            hybrid_score = self.clap_weight * float(clap_score) - self.decoder_weight * float(decoder_nll)
